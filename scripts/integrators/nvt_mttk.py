...
class MTTK(BaseIntegrator):
    ...
    @logger_wraps()
    def stage_1(self):
        if self.external.thermostat_parameters > 0:
    ...
    def nose_hoover_chain(self):
        ...
        for _ in range(time_step_divider):
            for i in range(n_ys):
                ...
                self.do_liouville_nvt_last(step=_step)
                self.do_liouville_nvt_not_last(step=_step, is_right=True)
                # TODO
                self.do_liouville_npt(step=_step)
                scale, _ = self.do_liouville_velocities(
                    step=_step,
                    scale=scale,
                )
                self.do_liouville_xis(step=_step)
                self.do_liouville_npt(step=_step)
                self.do_liouville_nvt_not_last(step=_step, is_right=False)
                self.do_liouville_nvt_last(step=_step)
    ...
    def get_g_nvt_0(self, kinetic_energy: float):
        return (
                       2 * kinetic_energy
                       - 3 * self.system.configuration.particles_number
    ...
    @logger_wraps()
    def get_g_npt(self, kinetic_energy: float):
    ...
    @logger_wraps()
    def do_liouville_nvt_last(self, step: float):
        self.nvt_factors[
            self.thermostats_number - 1
        ] += self.g_nvt[self.thermostats_number - 1] * step / 4

    @logger_wraps()
    def do_liouville_nvt_not_last(self, step: float, is_right: bool):
    ...
    @logger_wraps()
    def do_liouville_npt(self, step: float):
    ...
    @logger_wraps()
    def do_liouville_velocities(self, step: float, scale: float):
        _factor = np.exp(
            -step / 2 * (
                    self.nvt_factors[0]
    ...
    @logger_wraps()
    def do_liouville_xis(self, step: float):
